...
class ChordNode:
    ...
    #     def run_server():
    #         uvicorn.run(self.app, host=self.address.ip, port=int(self.address.ports[0]))
    ...
    def register_with_multicast_router(self):
        ...
        register_message = {
            "message": "REGISTER",
            "ip": self.address.ip,
            "port": self.address.ports[0]  # Puerto principal del servidor
        }
    ...
    def handle_heartbeat(self):
        ...
        heartbeat_socket.bind((self.address.ip, int(self.address.ports[4])))  # Un puerto específico para heartbeats
        heartbeat_socket.listen()

        while True:
            conn, addr = heartbeat_socket.accept()
            data = conn.recv(1024).decode('utf-8')
            if data == "HEARTBEAT":
                conn.send("ALIVE".encode('utf-8'))
            conn.close()
                ...
                if not nextID == self.nodeID :
                    ...
                    for sucessor in self.Sucessors:
                        if sucessor != self.nodeID:
                            ...
                            send_request((self.node_address[sucessor].ip,int(self.node_address[sucessor].ports[0])),data=data)
    ...
    def create_user(self, data):
        self.db.create_user(data["user_name"], data["user_email"], data["password"])

    def get_user(self, data):
        response = str(int(data["message"]) + 1)
        user_name, user_email = self.db.get_user_by_id(data["user_key"])
    ...
    def create_group(self, data):
        group_data = {
            ...
            "creator": data["user_key"]
        }
        self.db.create_group(group_data)
    ...
    def get_all_events(self, data):
        idevents, enames, datesc, datesf, states, visibs = self.db.get_events(data["user_key"])
    ...
    def delete_event(self, data):
        self.db.delete_event(data["user_key"], data["id_event"])
    
    def get_groups_belong_to(self, data):
        idsgroup, gnames, hierarchies = self.db.get_user_groups(data["user_key"])
