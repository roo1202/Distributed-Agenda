...
@router.post("/groups/{group_id}/user/{user_id}", response_model=GroupResponse)
def add_user_to_group_endpoint(group_id: int, user_id: int, db: Session = Depends(get_db)):
    group = add_user_to_group(db, user_id, group_id)
    if group is None:
        raise HTTPException(status_code=404, detail="Group or User not found")
    return group

# Eliminar un usuario de un grupo
@router.delete("/groups/{group_id}/user/{user_id}", response_model=GroupResponse)
def remove_user_from_group_endpoint(group_id: int, user_id: int, db: Session = Depends(get_db)):
    group = remove_user_from_group(db, user_id, group_id)
    if group is None:
        raise HTTPException(status_code=404, detail="Group or User not found")
    return group
...
def get_user_groups_endpoint(user_id: int, db: Session = Depends(get_db)):
...
def get_users_in_group_endpoint(group_id: int, db: Session = Depends(get_db)):
